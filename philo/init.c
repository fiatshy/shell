...
void	init_info(t_info *info, int ac, char **av)
{
	...
	while (i < (ac - 1))
	{
		...
		else if (temp[i] == 0 && av[i + 1][0] == '0')
		{
			printf("Zero philosopher is not allowed\n");
			exit(1);
		}
		i++;
	}
...
void	init_mutex(t_info *info)
{
	...
	info->last = malloc (sizeof(pthread_mutex_t));
	info->stop = malloc (sizeof(pthread_mutex_t));
	info->print = malloc (sizeof(pthread_mutex_t));
	info->repeat = malloc (sizeof(pthread_mutex_t));
	info->last_meal = malloc (sizeof(pthread_mutex_t));
	pthread_mutex_init(info->last, NULL);
	pthread_mutex_init(info->stop, NULL);
	pthread_mutex_init(info->print, NULL);
	pthread_mutex_init(info->repeat, NULL);
	pthread_mutex_init(info->last_meal, NULL);
}
