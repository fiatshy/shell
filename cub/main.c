...
void	render_frame(void *data)
{
	...
	while (x < WIDTH)
	{
		...
		if (side == 0)
		{
			...
			wallX = tx->pos_arr[1] + final_length * rayY;
		}
		else
		{
			final_length = distY - deltaY;
			wallX = tx->pos_arr[0] + final_length * rayX;
		}
		/* fix */
		wallX -= (int)wallX;
		...
		int	lineheight = (int)(HEIGHT / final_length);
		...
		for (int y=drawStart; y < drawEnd; y++)
		{
			...
			texPos += step;
			//printf("%d %f\n", texX, texY);
			int	color = tx->addr[TEXTURE_HEIGHT * texY + texX];
			put_pixel(tx->img, x, y, color);
		}
		//printf("==%f %d %d %d %d\n", wallX, texX, drawStart, drawEnd, lineheight);
		//printf("%f\n", final_length);
...
int	main(void)
{
	...
	int	xpm_width;
	int	xpm_height;
	void	*img_ptr = mlx_xpm_file_to_image(tx.mlx, "east.xpm", &xpm_width, &xpm_height);
